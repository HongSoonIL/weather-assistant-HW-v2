...
| 이름 | 역할 |
| --- | --- |
| 홍순일　<br/> (팀장) | PM, Tech Lead, LLM기반 응답 알고리즘 개편, 아두이노 & 라즈베리파이 회로 설계 및 연동 구현, 옷차림 이미지 분석 알고리즘 설계, 촬영 및 하드웨어 연동 프론트엔드 UI 구현, 날씨정보 시각화 알고리즘 설계 (홀로그램, 유리구슬 LED), Git관리 및 팀 코드 리뷰 |
